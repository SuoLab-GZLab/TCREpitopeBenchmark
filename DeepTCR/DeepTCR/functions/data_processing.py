...
def save_model_data_old(self,saver,sess,name,get,iteration=0):
...
def save_model_data(self, saver, sess, name, get, iteration=0):
    ...
    Args:
        ...
        get: TensorFlow operation or object to track.
        iteration: Integer to specify the iteration or version of the model.
    """
    print('saving models....')
    # Define the directory for saving checkpoints
    model_dir = os.path.join(self.Name, 'models', 'model_' + str(iteration))
    ...
    # Ensure the checkpoint is added to `all_model_checkpoint_paths`
    checkpoint_metadata_path = os.path.join(model_dir, 'checkpoint')
    if not os.path.exists(checkpoint_metadata_path):
        with open(checkpoint_metadata_path, 'w') as f:
            f.write(f'model_checkpoint_path: "{checkpoint_path}"\n')
            f.write(f'all_model_checkpoint_paths: "{checkpoint_path}"\n')

    # Save model metadata
    metadata_path = os.path.join(self.Name, 'models', 'model_type.pkl')
    with open(metadata_path, 'wb') as f:
        pickle.dump([
            name, get.name, self.use_alpha, self.use_beta,
            self.use_v_beta, self.use_d_beta, self.use_j_beta,
            self.use_v_alpha, self.use_j_alpha, self.use_hla, 
            self.use_hla_sup, self.keep_non_supertype_alleles,
            self.lb_v_beta, self.lb_d_beta, self.lb_j_beta,
            self.lb_v_alpha, self.lb_j_alpha, self.lb_hla, self.lb,
            self.ind, self.regression, self.max_length
        ], f)
