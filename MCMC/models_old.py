...
def _pairwise_distances(embeddings, squared=False):
    ...
    if not squared:
        mask = tf.cast(K.tf.equal(distances, 0.0), tf.float32)
...
def train(model_dict, data_dict, epochs, model_name, save_model_path,trainfileName, batch_size=BATCH_SIZE, period=10, choice_idx=None, name=None, auto_stop_n=None,):
    ...
    for i in range(epochs):
        ...
        exp.save(f'{save_model_path}/model_{model_name}_{trainfileName}.h5')  # add save .h5 
