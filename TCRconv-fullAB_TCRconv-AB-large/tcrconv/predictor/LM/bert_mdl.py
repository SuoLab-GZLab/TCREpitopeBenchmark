...
class ProtBertClassifier(pl.LightningModule):
    ...
    def __init__(self, hyperparams) -> None:
        ...
        if self._hparams.tune_epitope_specificity:
    ...
    def prepare_sample(self, sample: list, prepare_target: bool = True) -> (dict, dict):
        ...
        inputs = self.tokenizer.batch_encode_plus(sample["seq"],
                                                  add_special_tokens=self._hparams.special_tokens,
                                                  padding=True,
                                                  truncation=True,
                                                  max_length=self._hparams.max_length)
    ...
    def configure_optimizers(self):
        ...
        parameters = [
            {"params": self.classification_head.parameters()},
            {
                ...
                "lr": self._hparams.encoder_learning_rate,
            },
        ]
        optimizer = optim.Adam(parameters, lr=self._hparams.learning_rate)
        return [optimizer], []
...
def parse_arguments():
    ...
    hyperparams.__dict__.update(params)
    return hyperparams
...
def retrieve_model():
    ...
    model = ProtBertClassifier(hyperparams)
    return model
...
def extract_and_save_embeddings(model=None, data_f_n="vdj_human_unique_longs.csv", separator=",", sequence_col="long",
    ...
    inputs = model.tokenizer.batch_encode_plus(cropped_seqs[:],#[:10]
                                               ...
                                               padding=True,
                                               truncation=True,
                                               max_length=model._hparams.max_length)
    ...
    model.eval()
    file_index = 0
    print('seqs',len(sequences),sequences[0])
    print('inputs:',np.shape(inputs['input_ids']),np.shape(inputs['token_type_ids']),np.shape(inputs['token_type_ids']))
    for i in range(int(np.ceil(len(sequences)/ 100))):
        ...
        # print(np.shape(current_inputs['input_ids']))
        ...
        for seq_num in range(len(embedding)):
            ...
            start_Idx = 1 if model._hparams.special_tokens else 0
            end_Idx = seq_len + 1 if model._hparams.special_tokens else seq_len
...
def compute_embs(model, seqs, cdr3s=None):
    ...
    inputs = model.tokenizer.batch_encode_plus(cropped_seqs,
                                               add_special_tokens=model._hparams.special_tokens,
                                               padding=True,
                                               truncation=True,
                                               max_length=model._hyparams.max_length)
    embedding = model.forward(**inputs)
    embedding = embedding.cpu().numpy()
    embedding = remove_special_tkns(embedding, model._hparams.special_tokens)
