...
def get_embeddings(tcr_emb_dict,cdr3s,longs,cdr3max,device,append_onehot=False):
    # print(type(longs[0]))  # 可能会输出 <class 'numpy.str_'>
    ...
    if append_onehot:
        ...
        for i,(cdr3,tcr) in enumerate(zip(cdr3s,longs)):
            # print(f"tcr_emb_dict[{tcr}] shape:", torch.tensor(tcr_emb_dict[tcr]).shape)
...
def get_dataloaders(p,device,usetypes=['train']):
