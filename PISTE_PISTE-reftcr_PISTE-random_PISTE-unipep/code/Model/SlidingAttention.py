...
class DualInterAttention(nn.Module):
    ...
    def initial_position(self, name, device, len_q, len_k):#len_q is real length of peptide
        len_q = len_q.unsqueeze(1) - 1
        pep_maxlen=15
        if name == "HLA_pep":
            k_pos = torch.arange(34).type('torch.FloatTensor').unsqueeze(0).unsqueeze(0).to(device)
            q_pos_1 = 13. * (torch.arange(pep_maxlen).type('torch.FloatTensor').unsqueeze(0).unsqueeze(0).to(device) / len_q)  # 夹子1，长度为14
            q_pos_1[q_pos_1 > 13] = 1e4
            
            q_pos_2 = 9. * (torch.arange(pep_maxlen).type('torch.FloatTensor').unsqueeze(0).unsqueeze(0).to(device) / len_q) + 14.  # 底座，长度为10
            q_pos_2[q_pos_2 > 23] = 1e4
            
            q_pos_3 = 9 - 9. * (torch.arange(pep_maxlen).type('torch.FloatTensor').unsqueeze(0).unsqueeze(0).to(device) / len_q) + 24.  # 夹子2， 长度为10
            ...
            k_pos_1 = torch.arange(pep_maxlen).type('torch.FloatTensor').unsqueeze(0).unsqueeze(0).expand(B, 1, pep_maxlen).to(device)
    ...
    def generate_space_matrix(self, name, device, key_pos, query_positions, transpose=False):
        ...
        if name == "HLA_pep":
            ...
            key = key_pos.unsqueeze(-2)
            q_1 = query_positions[0].unsqueeze(-1)
            k_q_1 = torch.exp(-torch.square(q_1 - key[:, :, :, :15]) / (2 * self.sigma * self.sigma)) #change 11 to 15
            B, H, _, _ = k_q_1.shape
    ...
    def update_pos(self, amino_att, space_att, query_positions, key_pos, name):
        ...
        if name == "HLA_pep":
            ...
            full_att_1 = torch.softmax(amino_att[:, :, :, :15].clone().masked_fill_(mask[:, :, :, :15], -np.inf), dim=-1) #change 11 to 15
            full_att_1 = torch.where(torch.isnan(full_att_1), torch.full_like(full_att_1, 0), full_att_1)
            query_positions[0] = torch.matmul(full_att_1, key_pos[:, :, :15].unsqueeze(-1)).squeeze(-1) + (1 - full_att_1.sum(-1)) * query_positions[0] #change 11 to 15
    ...
    def forward(self, query, key, pep_attn_mask, hla_attn_mask, amino_mask, len_q, len_k):
        ...
        if (L_Q + L_K) == 49:# change 45 to 49
            name = "HLA_pep"
        elif (L_Q + L_K) == 79:# change 75 to 79
            name = "HLApep_TCR"
        else:
            name = None
