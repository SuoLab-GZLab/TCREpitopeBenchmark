...
class DecoderLayer(nn.Module):
    ...
    def forward(self, hla_inputs, pep_inputs, tcr_inputs, pep_attn_mask, tcr_attn_mask): # dec_inputs = enc_outputs
        ...
        tcr2pep_outputs, tcr2pep_attn = self.tcr2pep_attn(pep_inputs, tcr_inputs, tcr_attn_mask[:, :, :15, :]) #change 11 to 15
...
class Transformer(nn.Module):
    ...
    def forward(self, pep_inputs, hla_inputs, tcr_inputs):
        ...
        ################################################################################################################
        pep_pos = (torch.arange(15).type('torch.FloatTensor').to(self.device) + 1.) * (33 / 15.) - 0.5*(33 / 15.)#
        ...
        ################################################################################################################
        hla_dec_embedding = (binary_enc_outputs + binary_inter_outputs)[:, 15:, :]
        pep_dec_embedding = (binary_enc_outputs + binary_inter_outputs)[:, :15, :]
