...
def get_sequence_lists(datafile,organism,epi,cdr_types,delimiter,clip,lmax3=None,
                       ...
                       check_v='none',balance_controls=False,encoding='bytes',
                       alphabet_db_file_path='data/alphabeta_db.tsv'):
...
def loso(datafile,organism,epi,pc,cdr_types=[[],['cdr3']],l=[1.0],var=[1.0],
    ...
    for subject in subjects_u:
        ...
        with tf.Session(graph=tf.Graph()):
            ...
            if nZ == 0: # use VGP
                m = gpflow.models.VGP(X[I,:],y[I],kernel,gpflow.likelihoods.Bernoulli())(fix0119)
                #m = gpflow.models.VGP(tf.convert_to_tensor(X[I,:]),tf.convert_to_tensor(y[I]),kernel,gpflow.likelihoods.Bernoulli())
            else: # use SVGP
                # inducing locations by kmeans
                Z, mbs = select_Z_mbs(nZ,mbs,X[I,:])
                m = gpflow.models.SVGP(X[I,:],y[I],kernel,gpflow.likelihoods.Bernoulli(),Z=Z,minibatch_size=mbs)(fix0119)
                #m = gpflow.models.SVGP(tf.convert_to_tensor(X[I,:]),tf.convert_to_tensor(y[I]),kernel,gpflow.likelihoods.Bernoulli(),Z=Z,minibatch_size=mbs)
...
def train_classifier(datafile,organism,epi,pc, cdr_types=[[],['cdr3']],
                     ...
                     check_v='none',balance_controls=False, return_preds=False,
                     alphabet_db_file_path='data/alphabeta_db.tsv'):
    """
    ...
    with tf.Session(graph=tf.Graph()):
        ...
        if nZ == 0:
            ...
            #m = gpflow.models.VGP(tf.convert_to_tensor(X),tf.convert_to_tensor(y),kernel,gpflow.likelihoods.Bernoulli())
        else:
            # inducing locations by kmeans
